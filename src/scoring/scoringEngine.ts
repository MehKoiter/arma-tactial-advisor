...
import type {
  ...
  TransportHeloConfig,
  ReinforceConfig,
  InfantryDefendConfig,
  InfantryAssaultConfig,
} from './scoringConfig'
...
export function calcEnemyPressure(
  cap: CAP,
 ...
 */
export function calcContestedCentrality(
  cap: CAP,
  ownership: Record<string, Owner>,
  enemy: PlayerTeam,
): number {
 ...
 */
export function calcOverextension(
  cap: CAP,
  ownership: Record<string, Owner>,
  enemy: PlayerTeam,
): number {
...
export function bfsHopDistance(startId: string, targetId: string, caps: CAP[]): number {
 ...
 */
export function calcNotesBias(cap: CAP, notes: PositionNote[]): number {
  const tagged = notes.filter((n) => n.capId === cap.id)
  if (tagged.length === 0) return 0
  const avg = tagged.reduce((sum, n) => sum + n.rating, 0) / tagged.length
  return (avg - 3) / 2
}
...
export function scoreCandidates(
  ...
  const radioActive =
    radioAnalysis.hqValid || radioAnalysis.mobAnchored || radioAnalysis.onlineSet.size >= 2
  ...
  const scored: ScoredCAP[] = caps
    ...
    .map((cap) => {
      ...
      const movementFeasibility = calcMovementFeasibility(
        cap,
        lavPosition,
        caps,
        config.maxFeasibleHops,
        friendlySet,
      )
      const notesBias = calcNotesBias(cap, notes)
      const supplyProximity = calcSupplyProximity(
        cap,
        supplyPoints,
        config.supplyProximityRadiusMetres,
      )
      ...
      if (movementFeasibility > 0)
        rationale.push(
          `Reachable from current LAV position via the CAP graph (≤ ${config.maxFeasibleHops} adjacent CAPs)`,
        )
      if (chokepoint)
      ...
      else if (radioConnected) rationale.push('On the HQ radio network — active spawn/supply node')
      if (notesBias > 0.15)
        rationale.push(
          `Field notes rate this area positively (avg ${(notesBias * 2 + 3).toFixed(1)}/5)`,
        )
      else if (notesBias < -0.15)
        rationale.push(
          `Field notes flag this area as risky (avg ${(notesBias * 2 + 3).toFixed(1)}/5)`,
        )
      if (underAttackUrgency) rationale.push('⚠ Under active attack — urgent LAV support needed')
      if (attackingPressure > 0)
        rationale.push(
          `${attackingPressure} neighboring enemy CAP(s) flagged as attacking — coordinated push`,
        )
      if (supplyProximity > 0.1)
        rationale.push('Near supply depot(s) — strategic asset worth defending')
      if (rationale.length === 0) rationale.push('Low activity zone — safe repositioning option')
...
export function scoreAttackCandidates(
  ...
  const enemyRadioActive =
    enemyRadio.hqValid || enemyRadio.mobAnchored || enemyRadio.onlineSet.size >= 2
  ...
  const scored = caps
    ...
    .map((cap) => {
      ...
      const friendlyNeighbors = cap.neighbors.filter(
        (n) => (ownership[n] ?? 'neutral') === playerTeam,
      ).length
      ...
      const enemyNeighbors = cap.neighbors.filter(
        (n) => (ownership[n] ?? 'neutral') === enemy,
      ).length
      ...
      const noProjection =
        myRadioActive && !cap.neighbors.some((n) => myRadio.onlineSet.has(n)) ? 1 : 0
      ...
      if (majorBonus) rationale.push('Major base — high strategic value')
      if (notesBias > 0.15)
      ...
      if (momentum) rationale.push('Flagged as active assault target — player intent')
      ...
      if (chokepoint) rationale.push('Enemy chokepoint — capturing severs their network chain')
      if (enemyRadioChokepoint)
        rationale.push('Enemy radio chokepoint — capturing severs their HQ chain')
      if (enemyOffline) rationale.push('Enemy CAP is offline — no spawn/resupply; soft target')
      if (noProjection)
...
export function scoreStrikeTargets(
  ...
  const scored = caps
    ...
    .map((cap) => {
      ...
      if (majorBonus) rationale.push('Major base — priority strike target')
      if (momentum) rationale.push('⚔ Ground assault in progress — CAS requested')
      if (casRelief > 0)
        rationale.push(`Suppressing this relieves ${casRelief} friendly CAP(s) under attack`)
      if (rangeScore > 0) rationale.push(`Within strike range from current position`)
      if (notesBias > 0.15)
...
export function scoreResupplyTargets(
  ...
  const scored = caps
    ...
    .map((cap) => {
      ...
      const rationale: string[] = []
      if (underAttackBonus) rationale.push('⚠ Under active attack — urgent troop drop needed')
      if (enemyNeighbors > 0)
        rationale.push(`${enemyNeighbors} enemy-adjacent neighbor(s) — frontline LZ`)
      if (majorBonus) rationale.push('Major base — large reinforcement capacity')
      if (rangeScore > 0) rationale.push('Within transport range from current position')
      if (notesBias > 0.15)
      ...
      if (supplyProximity > 0.1) rationale.push('Near supply depot(s) — troops can rearm locally')
      if (rationale.length === 0) rationale.push('Rear-area base — safe reinforcement option')
...
export function scoreReinforceTargets(
  ...
  const scored = caps
    ...
    .map((cap) => {
      ...
      const enemySupporters = cap.neighbors.filter(
        (n) => (ownership[n] ?? 'neutral') === enemy,
      ).length
      const isolation = cap.neighbors.length ? 1 - enemySupporters / cap.neighbors.length : 1

      const majorBonus = cap.type === 'major' ? 1 : 0

      // How many friendly CAPs are adjacent — more = better troop landing support
      const friendlySupport = cap.neighbors.filter(
        (n) => (ownership[n] ?? 'neutral') === playerTeam,
      ).length
      ...
      if (isolation >= 0.6) rationale.push('Target is isolated — high capture probability')
      else if (isolation < 0.4) rationale.push('Enemy reinforcements nearby — act quickly')
      if (majorBonus) rationale.push('Major base — high strategic value')
      ...
      if (supplyProximity > 0.1) rationale.push('Near supply depot(s) — high-value capture target')
...
export function scoreInfantryDefend(
  ...
  const scored = caps
    ...
    .map((cap) => {
      ...
      const rationale: string[] = []
      if (underAttackBonus) rationale.push('⚠ Under active attack — garrison immediately')
      if (enemyNeighbors > 0)
        rationale.push(`${enemyNeighbors} enemy-adjacent neighbor(s) — frontline garrison`)
      if (majorBonus) rationale.push('Major base — buildings to fortify')
      if (rangeScore > 0) rationale.push('Within on-foot range from current position')
      if (notesBias > 0.15)
      ...
      if (supplyProximity > 0.1) rationale.push('Near supply cache — squad can rearm locally')
      if (rationale.length === 0) rationale.push('Quiet sector — low-priority garrison')
...
export function scoreInfantryAssault(
  ...
  const scored = caps
    ...
    .map((cap) => {
      ...
      const rationale: string[] = []
      if (uncontested) rationale.push('Neutral CAP — uncontested capture')
      if (momentum) rationale.push('⚔ Already engaging — keep momentum')
      if (friendlySupport > 0)
      ...
      if (majorPenalty) rationale.push('Major base — hard to clear on foot, prefer fire support')
      if (rangeScore > 0) rationale.push('Within on-foot range from current position')
      if (notesBias > 0.15)
      ...
      if (supplyProximity > 0.1) rationale.push('Capturing grants supply access')
      if (rationale.length === 0)
        rationale.push(
          isNeutral ? 'Neutral CAP within foot range' : 'Capturable enemy CAP within foot range',
        )
